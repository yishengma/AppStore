...
/**
 * Created by PirateHat on 2019/2/20.
 */

public class DownloadTask1 extends AsyncTask<String, Integer, Integer> {
    ...
    public DownloadTask1(DownloadListener listener) {
...
//                .build();
        ...
        try {
            ...
            if (response != null) {
                is = new ByteArrayInputStream(response.getByteArray());
            ...
            CloseQuicklyUtil.close(savedFile);
...
//        return 0;
        ...
        try {
            ...
            Response response = client.newCall(request).execute();
            if (!response.isCanceled() && response.mStatus == Response.OK) {
                mUrl = request.url();
